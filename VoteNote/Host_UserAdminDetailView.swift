...
struct HostUserDetailView: View {
  ...
  var body: some View {
                    ...
                    List {
                        ForEach(songQueue.musicList) { song in
                            if song.addedBy == user.name { //we should probably have a better way to make this comparision
                                QueueEntry(curSong: song, selectedSong: selectedSong, songQueue: songQueue, isViewingUser: hostControllerHidden, isDetailView: true, isUserQueue: false, hostControllerHidden: hostControllerHidden)
                            }
                        }
                    }
                }
    ...
    }.navigate(to: Host_QueuePageView(hostControllerHidden: hostControllerHidden), when: $shouldReturn).onAppear(perform: {
        shouldReturn = false
